...
{ id: 2, title: "Yoga Flow", description: "Increase flexibility and joint mobility, develop functional balance", duration: "8 weeks", difficulty: "Beginner", caloriesBurn: "200-300", workouts: 5, cost: "500₹" },
{ id: 3, title: "Cardio Blast", description: "High-intensity cardio to boost endurance and burn fat", duration: "6 weeks", difficulty: "Advanced", caloriesBurn: "600-800", workouts: 5, cost: "800₹" }];
...
export function WorkoutPrograms() {
  ...
  return (
    <>
      {/* Tabs */}
      <div className="flex gap-2 mb-8 overflow-x-auto ml-15 mt-3.5">
      ...
      {activeTab === "programs" ? (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mx-15 mb-3.5">
            {programs.map(program => (
              <div key={program.id} className="bg-white rounded-2xl shadow-lg p-6 flex flex-col hover:shadow-xl transition"
                ...
                <div className="flex justify-between items-center"> <h3 className="text-xl font-semibold text-gray-900 mb-2">{program.title}</h3><span className="mb-3 w-23 py-2 rounded-full font-semibold text-white bg-linear-to-r from-blue-600 to-indigo-600 hover:opacity-90 text-center">{program.cost}</span></div>
                <span className="w-25 inline-block mb-4 px-3 py-1 text-sm rounded-full bg-blue-100 text-blue-700">
                  {program.difficulty}
                </span>
